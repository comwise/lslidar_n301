...
bool LslidarN301Driver::loadParameters() {
  ...
  pnh.param<int>("device_port", udp_port, 2368);
  inet_aton(device_ip_string.c_str(), &device_ip);
  ROS_INFO_STREAM("Opening UDP socket: " << device_ip_string << ": " << udp_port);
  return true;
}
...
bool LslidarN301Driver::createRosIO() {
  ...
  // packet rate.
  double diag_freq = 16*20000.0 / (12*32);
  //diag_freq = 10;
  ...
  ROS_INFO("expected frequency: %.3f (Hz)", diag_freq);
    ...
    diag_topic.reset(new TopicDiagnostic(
                         ...
                         FrequencyStatusParam(&diag_min_freq, &diag_max_freq, 0.5, 10),
...
bool LslidarN301Driver::openUDPPort() {
    ...
    if (socket_id == -1) {
        ROS_ERROR_STREAM("create socket failed");
        return false;
    }
    ...
    if (bind(socket_id, (sockaddr *)&my_addr, sizeof(sockaddr)) == -1) {
        ROS_ERROR_STREAM("bind socket failed");
        close(socket_id);
        return false;
    }
    ROS_INFO_STREAM("bind socket success");

    if (fcntl(socket_id, F_SETFL, O_NONBLOCK|FASYNC) < 0) {
        ROS_ERROR_STREAM("set non-block mode failed");
        close(socket_id);
        return false;
    }
    ROS_INFO_STREAM("set non-block mode ok");
...
bool LslidarN301Driver::initialize() {
    ...
    ROS_INFO("Initialize lslidar n301 success");
