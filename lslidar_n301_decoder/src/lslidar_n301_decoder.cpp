...
LslidarN301Decoder::LslidarN301Decoder(ros::NodeHandle& n, ros::NodeHandle& pn) :
    nh(n),
    pnh(pn),
    use_gps_ts(false),
    is_first_sweep(true),
...
bool LslidarN301Decoder::loadParameters() {
    ...
    ROS_INFO_STREAM("old_point_num: " << point_num);

    pnh.param<double>("min_range", min_range_, 0.5);
    pnh.param<double>("max_range", max_range_, 100.0);
    ROS_INFO_STREAM("min_range: " << min_range_ << ", max_range: " << max_range_);

    pnh.param<double>("angle_disable_min", angle_disable_min, -1);
    pnh.param<double>("angle_disable_max", angle_disable_max, -1);
    ROS_INFO_STREAM("angle_disable_min: " << angle_disable_min << ", angle_disable_max: " << angle_disable_max);

    pnh.param<double>("start_angle", start_angle_, 0);
    pnh.param<double>("end_angle", end_angle_, 360);
    ROS_INFO_STREAM("start_angle: " << start_angle_ << ", end_angle: " << end_angle_);

    pnh.param<double>("frequency", frequency_, 10.0);
    pnh.param<double>("resolution", resolution_, 0.18);
    ROS_INFO_STREAM("frequency: " << frequency_ << ", resolution: " << resolution_);

    pnh.param<bool>("inverted", inverted_, false);
    pnh.param<bool>("intensity", intensity_, true);
    ROS_INFO_STREAM("inverted: " << inverted_ << ", intensity: " << intensity_);

    pnh.param<bool>("publish_point_cloud", publish_point_cloud_, true);
    ROS_INFO_STREAM("publish_point_cloud: " << publish_point_cloud_);

    pnh.param<bool>("use_gps_ts", use_gps_ts, false);
    ROS_INFO_STREAM("Using GPS timestamp:" << use_gps_ts);

    pnh.param<string>("child_frame_id", child_frame_id, "lslidar");
    ROS_INFO_STREAM("child_frame_id: " << child_frame_id);

    if(std::fabs(resolution_) <= 0.001) {
        resolution_ = 0.18;
    }

#ifdef NEW_WAY
    angle_clockwise_flag_ = false;
    double start_temp = start_angle_ ;
    double end_temp = end_angle_;
    if ((std::fabs(start_temp - end_temp) < 0.001) || start_temp < -0.001 || end_temp < -0.001) {
        start_temp = 0.0;
        end_temp = 360.0;
    }
    if((start_temp > 0.001) && (end_temp > 0.001) && (start_temp - end_temp > 0.01)) {
        angle_clockwise_flag_ = true;
    }
    ROS_INFO_STREAM("angle_clockwise_flag: " << angle_clockwise_flag_);


#ifdef NEW_WAY_SPE 
    double point_float = (end_temp - start_temp) / resolution_;
    point_num = int(point_float + 0.5);
    ROS_INFO_STREAM("new_point_num: " << point_num);
#endif

    start_angle_ = start_temp;
    end_angle_ = end_temp;
    ROS_INFO_STREAM("adjust>>start_angle: " << start_angle_ << ", end_angle: " << end_angle_);
#endif
...
void LslidarN301Decoder::publishScan()
{
    ...
    scan->angle_increment = (double)resolution_ * D2R;
    scan->time_increment = 0.00005;  //s
	...
	{
#ifndef NEW_WAY
		if((i >= (360-end_angle_)*point_num/360) && (i < (360-start_angle_)*point_num/360)) {
			scan->ranges[i] = std::numeric_limits<float>::infinity();
            scan->intensities[i] = std::numeric_limits<float>::infinity();
        }
#else
        if((!angle_clockwise_flag_ && (i < start_angle_*point_num/360) || (i > end_angle_*point_num/360)) ||
           (angle_clockwise_flag_ && (i < start_angle_*point_num/360) && (i >= end_angle_*point_num/360))) {
			scan->ranges[i] = std::numeric_limits<float>::infinity();
            scan->intensities[i] = std::numeric_limits<float>::infinity();
        }
#endif
...
void LslidarN301Decoder::packetCallback(
    ...
    for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
        for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
			if(firings[fir_idx].intensity[scan_idx] < 3) continue;													  
